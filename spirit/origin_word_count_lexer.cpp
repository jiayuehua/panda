...
struct word_count_tokens : lex::lexer<Lexer>
{
      ...
      , any(".", ID_ANY)
    {
		...
		std::string ini("INITIAL");
		std::string secondstate("second");
		std::string thirdstate("third");
		word.collect(*this, ini,secondstate);
		any.collect(*this, secondstate, thirdstate);
        this->self 
			=   word  [++ref(w), ref(c) += distance(_start, _end), std::cout<<_state<<" "]
            |   eol   [++ref(c), ++ref(l), std::cout<<_state<<" "] 
			|   any   [++ref(c),std::cout<<_state<<" "]
            ;
    ...
    std::size_t c, w, l;
	lex::token_def<std::string> word, eol, any;
};
//]

///////////////////////////////////////////////////////////////////////////////
//[wcl_main
int origin_word_count_main(int argc, char* argv[])
{
